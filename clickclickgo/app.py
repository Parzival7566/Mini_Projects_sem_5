...
def login_page():
    if request.method == 'POST':
        # Check if the form data is for host login
        if 'host_username' in request.form and 'host_password' in request.form:
            username = request.form.get('host_username')
            password = request.form.get('host_password')
            user = host_details.find_one({'username': username})
            if user and check_password_hash(user['password'], password):
                session['username'] = username
                session['user_type'] = 'host'
                return redirect(f'/{username}/HostDashboard')
            else:
                flash('Invalid username or password', 'danger')
                return redirect('/')
        # Check if the form data is for host signup
            ...
            if user:
                ...
                return redirect('/')
        # Check if the form data is for attendee login
        elif 'attendee_username' in request.form:
            username = request.form.get('attendee_username')
            session['username'] = username
...
@app.route('/<username>/HostDashboard')
def admin_page(username):
    ...
    if session_username and session_username == username:
        ...
        if user:
            return render_template('HostDashboard.html', username=session_username)
        else:
